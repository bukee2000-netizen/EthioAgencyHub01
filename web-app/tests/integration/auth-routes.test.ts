...
(HAS_MYSQL_DB ? describe : describe.skip)('Auth API Routes', () => {
  ...
  describe('POST /api/auth/login', () => {
    ...
    it('should allow demo credentials even when the database is configured', async () => {
      const req = new Request('http://localhost/api/auth/login', {
        method: 'POST',
        body: JSON.stringify({ email: 'admin@ethioagency.com', password: 'admin123' })
      });
      const res = await LoginPOST(req);
      expect(res.status).toBe(200);
      const body = await res.json();
      ...
      expect(body).toHaveProperty('data');
      expect(body.data).toHaveProperty('user');
      expect(body.data.user).toHaveProperty('email', 'admin@ethioagency.com');
      expect(body.data).toHaveProperty('csrfToken');
      expect(body.data).toHaveProperty('isDemo', true);
    });
  });
