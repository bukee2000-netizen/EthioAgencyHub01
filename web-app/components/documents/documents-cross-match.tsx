...
export function DocumentsCrossMatch() {
  ...
  const runCrossMatch = () => {
    ...
    const res: CheckResult[] = employees.slice(0, 30).map((e: any) => {
      ...
      if (simulateExpiryFail) errors.push(`Passport expires ${new Date(Date.now() + 60 * 86400000).toLocaleDateString()} â€“ within 6 months of travel.`);
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-gradient-to-br from-white via-green-50/30 to-white p-8 shadow-sm dark:shadow-soft-dark flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-ink dark:text-ink-dark">Cross-Match: Pre-Flight Safety Gate</h1>
        </div>
        <button onClick={runCrossMatch} className="flex items-center gap-2 rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-brand-700 shadow-sm dark:shadow-soft-dark">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className={`rounded-2xl border-2 p-5 shadow-sm dark:shadow-soft-dark ${passCount > 0 ? 'border-emerald-300 bg-gradient-to-br from-emerald-50 to-emerald-100/50' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800'}`}>
        ...
        <div className={`rounded-2xl border-2 p-5 shadow-sm dark:shadow-soft-dark ${failCount > 0 ? 'border-red-300 bg-gradient-to-br from-red-50 to-red-100/50' : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800'}`}>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <button onClick={() => setStatusFilter('all')} className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all ${statusFilter === 'all' ? 'bg-brand-600 text-white shadow-sm dark:shadow-soft-dark' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}>All</button>
        <button onClick={() => setStatusFilter('pass')} className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all ${statusFilter === 'pass' ? 'bg-emerald-600 text-white shadow-sm dark:shadow-soft-dark' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}><CheckCircle2 className="h-4 w-4 inline-block mr-1" />Safe</button>
        <button onClick={() => setStatusFilter('fail')} className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all ${statusFilter === 'fail' ? 'bg-red-600 text-white shadow-sm dark:shadow-soft-dark' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}><AlertCircle className="h-4 w-4 inline-block mr-1" />Errors</button>
      </div>
      ...
      {results.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-16 text-center shadow-sm dark:shadow-soft-dark">
          ...
          <p className="font-medium text-slate-500 dark:text-slate-400">No data to check</p>
        ...
        <div className="space-y-4">
          {filtered.map(r => {
            ...
            return (
              <div key={r.employeeId} className={`rounded-2xl border-2 shadow-sm dark:shadow-soft-dark overflow-hidden transition-all ${r.allPass ? 'border-emerald-200 bg-white dark:bg-slate-800' : 'border-red-200 bg-red-50/30'}`}>
                <div className="px-6 py-4 flex items-center justify-between">
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-bold text-ink dark:text-ink-dark">{r.name}</p>
                  ...
                  <div className="px-6 py-5 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-200 dark:border-slate-700">
                    <div className="grid gap-5 md:grid-cols-3">
                      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                        <h4 className="font-bold text-ink dark:text-ink-dark text-sm mb-3 flex items-center gap-2"><FileText className="h-4 w-4 text-blue-600" /> Document Data</h4>
                        <div className="space-y-2 text-xs">
                          <div><span className="text-slate-500 dark:text-slate-400">Employee:</span> <span className="font-medium">{r.name}</span></div>
                          <div className="flex items-center gap-2 mt-2">
                            <span className={`w-3 h-3 rounded-full ${r.nameMatch === 'pass' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                            <span className="text-slate-500 dark:text-slate-400">Passport Name:</span> <span className="font-medium">{r.name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className={`w-3 h-3 rounded-full ${r.nameMatch === 'pass' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                            <span className="text-slate-500 dark:text-slate-400">Ticket Name:</span> <span className="font-medium">{r.name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className={`w-3 h-3 rounded-full ${r.nameMatch === 'pass' ? 'bg-emerald-500' : 'bg-red-500'}`} />
                            <span className="text-slate-500 dark:text-slate-400">Visa Name:</span> <span className="font-medium">{r.name}</span>
                          </div>
                        </div>
                      </div>
                      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                        <h4 className="font-bold text-ink dark:text-ink-dark text-sm mb-3 flex items-center gap-2"><Plane className="h-4 w-4 text-purple-600" /> Harmony Check</h4>
                        <div className="space-y-3 text-xs">
                          <div className={`p-2 rounded-lg ${r.visaCountryMatch === 'pass' ? 'bg-emerald-50' : 'bg-red-50'}`}>
                            ...
                            {r.visaCountryMatch === 'pass' ? <span className="text-emerald-600 font-medium">âœ“ Match</span> : <span className="text-red-600 font-medium">âœ— Mismatch</span>}
                          </div>
                          <div className={`p-2 rounded-lg ${r.passportMatch === 'pass' ? 'bg-emerald-50' : 'bg-red-50'}`}>
                            ...
                            {r.passportMatch === 'pass' ? <span className="text-emerald-600 font-medium">âœ“ Match</span> : <span className="text-red-600 font-medium">âœ— Mismatch</span>}
                          </div>
                        </div>
                      </div>
                      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                        <h4 className="font-bold text-ink dark:text-ink-dark text-sm mb-3 flex items-center gap-2"><Calendar className="h-4 w-4 text-amber-600" /> Expiry Countdown</h4>
                        <div className="space-y-3 text-xs">
                          <div className={`p-2 rounded-lg ${r.passportExpiryOk === 'pass' ? 'bg-emerald-50' : 'bg-red-50'}`}>
                            <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Passport Expires:</span><span className="font-medium">{new Date(r.passportExpiry).toLocaleDateString()}</span></div>
                            <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Travel Date:</span><span className="font-medium">{new Date(r.departureDate).toLocaleDateString()}</span></div>
                            {r.passportExpiryOk === 'pass'
                              ? <span className="text-emerald-600 font-medium">âœ“ Valid for travel ({'>'}6 months)</span>
                              : <span className="text-red-600 font-medium">âœ— Expiring within 6 months â€“ Flagged!</span>}
                          </div>
                          <div className={`p-2 rounded-lg ${r.visaExpiryOk === 'pass' ? 'bg-emerald-50' : 'bg-red-50'}`}>
                            <div className="flex justify-between"><span className="text-slate-500 dark:text-slate-400">Visa Expires:</span><span className="font-medium">{new Date(r.visaExpiry).toLocaleDateString()}</span></div>
                            {r.visaExpiryOk === 'pass' ? <span className="text-emerald-600 font-medium">âœ“ Valid</span> : <span className="text-red-600 font-medium">âœ— Expiring</span>}
                          </div>
                        </div>
                      </div>
                    </div>
