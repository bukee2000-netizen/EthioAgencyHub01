    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-semibold text-ink">WhatsApp Notifications</h3>
        </div>
        {loading ? (
      ...
      {!status?.configured && (
        <div className="mb-4 rounded-xl bg-amber-50 border border-amber-200 p-4 text-sm text-amber-800">
          ...
          <p>Set <code className="bg-amber-100 px-1 rounded">WHATSAPP_PHONE_ID</code>, <code className="bg-amber-100 px-1 rounded">WHATSAPP_ACCESS_TOKEN</code>, and <code className="bg-amber-100 px-1 rounded">WHATSAPP_VERIFY_TOKEN</code> in your <code className="bg-amber-100 px-1 rounded">.env</code> file to enable WhatsApp notifications.</p>
        </div>
      )}
      ...
      {status?.configured && (
        <div className="space-y-4">
          ...
          <div className="border-t border-slate-100 pt-4">
            <p className="text-sm font-medium text-ink mb-3">Send Test Message</p>
            <div className="space-y-3">
              <input type="text" placeholder="Phone number (e.g., +251911234567)" value={testPhone}
                onChange={(e) => setTestPhone(e.target.value)}
                className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm focus:border-green-500 focus:outline-none" />
