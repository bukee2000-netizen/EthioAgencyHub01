...
export function AdminUsers() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-ink dark:text-ink-dark">User Management</h1>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 bg-gradient-to-br from-yellow-50 to-yellow-100/50">
          <p className="text-sm text-slate-600 dark:text-slate-300">Inactive</p>
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 bg-gradient-to-br from-blue-50 to-blue-100/50">
          <p className="text-sm text-slate-600 dark:text-slate-300">Pending Approval</p>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden">
        <div className="border-b border-slate-200 dark:border-slate-700 px-6 py-4">
          <h3 className="text-lg font-bold text-ink dark:text-ink-dark">Users List</h3>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                ...
                <th className="px-6 py-3 text-left font-semibold text-slate-600 dark:text-slate-300">Last Login</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                  <td className="px-6 py-4 font-semibold text-slate-700 dark:text-slate-200">{user.name}</td>
                  ...
                  <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{user.lastLogin}</td>
                </tr>
              ))}
            </tbody>
          </table>
