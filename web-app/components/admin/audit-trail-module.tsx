    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm dark:shadow-soft-dark">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-ink dark:text-ink-dark">Audit Trail</h2>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-5">
        <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 p-4 text-center">
          <p className="text-xs font-medium text-slate-600 dark:text-slate-300">Total Events</p>
        ...
        <div className="rounded-2xl bg-gradient-to-br from-red-50 to-red-100 p-4 text-center">
          <p className="text-xs font-medium text-slate-600 dark:text-slate-300">Critical</p>
        ...
        <div className="rounded-2xl bg-gradient-to-br from-amber-50 to-amber-100 p-4 text-center">
          <p className="text-xs font-medium text-slate-600 dark:text-slate-300">Warnings</p>
        ...
        <div className="rounded-2xl bg-gradient-to-br from-blue-50 to-cyan-100 p-4 text-center">
          <p className="text-xs font-medium text-slate-600 dark:text-slate-300">Info</p>
        ...
        <div className="rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 p-4 text-center">
          <p className="text-xs font-medium text-slate-600 dark:text-slate-300">Unique Users</p>
          <p className="mt-2 text-3xl font-bold text-slate-700 dark:text-slate-200">{new Set(auditTrail.map(a => a.user)).size}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {filteredTrail.map((entry, idx) => (
          <div key={entry.id} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 hover:shadow-lg transition-shadow">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                ...
                <p className="font-medium text-ink dark:text-ink-dark">{entry.description}</p>
                <div className="mt-3 flex flex-wrap gap-4 text-sm">
                  <div>
                    <span className="text-slate-600 dark:text-slate-300">User: </span>
                    <span className="font-medium text-ink dark:text-ink-dark">{entry.user}</span>
                  </div>
                  <div>
                    <span className="text-slate-600 dark:text-slate-300">Resource: </span>
                    <span className="font-medium text-ink dark:text-ink-dark">{entry.resource}</span>
                  </div>
                  {entry.changes && (
                    <div>
                      <span className="text-slate-600 dark:text-slate-300">Changes: </span>
              ...
              <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition flex-shrink-0">
      ...
      {filteredTrail.length === 0 && (
        <div className="rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 p-12 text-center">
          ...
          <p className="text-slate-600 dark:text-slate-300 font-medium">No audit entries found</p>
