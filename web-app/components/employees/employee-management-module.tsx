...
export function EmployeeManagementModule() {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      addToast({ title: 'Error', description: 'Search failed. Please try again.', type: 'error' });
      setSearchResults([]);
    } finally {
      setSearching(false);
    }
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      ...
      default:
        return { color: 'bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700', label: status || 'Unknown' };
    }
  };
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-gradient-to-br from-white via-brand-50/30 to-white p-8 shadow-sm dark:shadow-soft-dark">
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
          <div>
            <h2 className="text-3xl font-extrabold text-ink dark:text-ink-dark">Employee Management</h2>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <div className="relative flex-1 lg:w-64">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-300 dark:border-slate-600 py-2.5 pl-9 pr-8 text-sm focus:border-brand-500 focus:outline-none"
              />
              {searchQuery && (
                <button
                  onClick={() => { setSearchQuery(''); setShowSearch(false); setSearchResults([]); }}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300"
                >
                  <X className="h-4 w-4" />
                </button>
              )}
        ...
        {showSearch && (
          <div className="mt-4 max-h-80 overflow-auto rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-lg">
            {searching ? (
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
                {searchResults.map((emp) => (
                  ...
                  >
                    <div>
                      <p className="font-semibold text-ink dark:text-ink-dark">{emp.name}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{emp.role} • {emp.destination}</p>
                    </div>
      ...
      <div>
        <h3 className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-4">Employee Pipeline</h3>
        <div className="grid gap-3 grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
          {[
            { label: 'Total', value: stats.total, bg: 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700', text: 'text-slate-800 dark:text-slate-100', icon: Users },
            ...
            <div
              key={item.label}
              className={`rounded-2xl border p-5 text-center ${item.bg} shadow-sm dark:shadow-soft-dark hover:shadow-md transition-shadow`}
            >
              <item.icon className={`h-5 w-5 mx-auto mb-2 ${item.text} opacity-60`} />
              <p className={`text-3xl font-extrabold ${item.text}`}>{item.value}</p>
              <p className="mt-1 text-xs font-bold text-slate-600 dark:text-slate-300">{item.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm dark:shadow-soft-dark">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold text-slate-700 dark:text-slate-200">Deployment Rate</p>
            <div className="p-2 rounded-lg bg-emerald-50"><CheckCircle2 className="h-5 w-5 text-emerald-600" /></div>
          </div>
          <p className="text-4xl font-extrabold text-ink dark:text-ink-dark">{deploymentRate}%</p>
          <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
            {stats.deployed} of {stats.total} successfully deployed abroad
          </p>
          <div className="mt-4 h-2.5 w-full rounded-full bg-slate-100 dark:bg-slate-700/50 overflow-hidden">
            <div
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm dark:shadow-soft-dark">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold text-slate-700 dark:text-slate-200">Readiness Rate</p>
            <div className="p-2 rounded-lg bg-blue-50"><TrendingUp className="h-5 w-5 text-blue-600" /></div>
          </div>
          <p className="text-4xl font-extrabold text-ink dark:text-ink-dark">{readinessRate}%</p>
          <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm dark:shadow-soft-dark">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold text-slate-700 dark:text-slate-200">In Process</p>
            <div className="p-2 rounded-lg bg-amber-50"><Clock className="h-5 w-5 text-amber-600" /></div>
          </div>
          <p className="text-4xl font-extrabold text-ink dark:text-ink-dark">{stats.documentReview + stats.interviewUploaded}</p>
          <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
      ...
      {/* Quick Actions */}
      <div>
        <h3 className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-4">Quick Actions</h3>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Link
            href="/employee-management/registration"
            className="group rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 hover:border-brand-300 hover:bg-brand-50/40 transition-all shadow-sm dark:shadow-soft-dark hover:shadow-md"
          >
            ...
            <h4 className="mt-4 font-bold text-ink dark:text-ink-dark text-lg">New Registration</h4>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Start the employee registration wizard</p>
          </Link>
          ...
          >
            ...
            <h4 className="mt-4 font-bold text-ink dark:text-ink-dark text-lg">View All Profiles</h4>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Browse and filter all employees</p>
          </Link>
          ...
          >
            ...
            <h4 className="mt-4 font-bold text-ink dark:text-ink-dark text-lg">Generate CV</h4>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Create professional CVs for employees</p>
          </Link>
          ...
          >
            ...
            <h4 className="mt-4 font-bold text-ink dark:text-ink-dark text-lg">CV Database</h4>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Search, filter, and match CVs</p>
          </Link>
        </div>
      </div>
      ...
      {/* Error Display */}
      {error && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-5 shadow-sm dark:shadow-soft-dark">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm dark:shadow-soft-dark overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-100 bg-slate-50 dark:bg-slate-800/50/50 flex items-center justify-between">
          <h3 className="text-lg font-bold text-ink dark:text-ink-dark">Recent Registrations</h3>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
          {loading ? (
            ...
            <p className="text-center text-slate-500 dark:text-slate-400 py-12 font-medium">No recent employees found.</p>
          ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-ink dark:text-ink-dark">{emp.name || 'Unknown'}</p>
...
{emp.role && <span className="flex items-center gap-1"><Briefcase className="h-3 w-3" /> {emp.role}</span>}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Link href={`/employee-management/${emp.id}`} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 dark:text-slate-500 hover:text-brand-600 transition-colors">
