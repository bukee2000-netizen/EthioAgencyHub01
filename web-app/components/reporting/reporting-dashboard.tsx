...
export function ReportingDashboard() {
  ...
  const handleExport = async (type: string) => {
    try {
      ...
      addToast({ title: 'Error', description: 'Failed to export report. Please try again.', type: 'error' });
    }
  };
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm dark:shadow-soft-dark">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400">Verified Documents</p>
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm dark:shadow-soft-dark">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400">Scheduled Travels</p>
      ...
      <section className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm dark:shadow-soft-dark">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-4">
          {['employees', 'documents', 'travels', 'pilgrims'].map(type => (
            <button key={type} onClick={() => handleExport(type)} className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 dark:border-slate-700 py-3 font-medium hover:bg-slate-50 dark:hover:bg-slate-700/50">
